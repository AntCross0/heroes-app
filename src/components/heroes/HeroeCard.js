...
const HeroeCard = ({ id, superhero, publisher, alter_ego, first_appearance, characters }) => {
    ...
    return (
        <Link to={`/hero/${id}`} key={id}
            className=' bg-white transition-all group hover:scale-105 h-auto min-[380px]:h-[200px] w-full min-[380px]:w-11/12 min-[480px]:w-[26rem] rounded drop-shadow-sm overflow-hidden'>
            <div
                style={{
            ...
            <div className=' z-30 grid grid-cols-1 min-[380px]:grid-cols-[70%,30%] h-full'>
                <div className='relative transition-color z-30 flex flex-col p-4 w-full h-full justify-between'>
                    <div className='space-y-1'>
                        ...
                        <h4 className=' font-bold text-2xl delay-300 group-hover:text-white truncate'>{superhero}</h4>
                ...
                <div className='rounded z-30 justify-self-end '>
                    <img className='h-full w-auto' alt={superhero} src={img} />
                </div>
